...
def generate(output_dir, sizes, num_instances, seed, epsilon,
    ...
    # The family-realizable class ("polytree-fr") has the same topology but
    # places the extra marginals on root atoms only, so every sentence is
    ...
    graph_type = "polytree-fr" if family_realizable else "polytree"
    prefix = "polytree_fr" if family_realizable else "polytree"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--family-realizable", action="store_true",
                        help="Generate the family-realizable class "
                             "'polytree-fr' (extra marginals on root atoms "
                             "only, so Credal VE / Interval BP are exact); "
                             "files prefixed polytree_fr_.")
