"""Generate chain LCN benchmark instances of increasing size."""
...
def generate(output_dir, sizes, num_instances, seed, epsilon,
             max_vars, num_extras, max_component_size, verbosity):
    ...
    for n in sizes:
        ...
        for i, lcn in enumerate(instances):
            fname = os.path.join(output_dir, f"chain_n{n}_{i + 1}.lcn")
            gen.save(lcn, fname)
            if verbosity > 0:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate chain LCN benchmark instances.")
    parser.add_argument("--output-dir", type=str, default="benchmarks/chain",
                        help="Output directory (default: benchmarks/chain)")
    ...
    parser.add_argument("--max-component-size", type=int, default=3,
                        help="Max variables per chain component (default: 3)")
    parser.add_argument("--verbosity", type=int, default=1,
                        help="Verbosity level (default: 1)")
    args = parser.parse_args()
    generate(args.output_dir, args.sizes, args.num_instances, args.seed,
             args.epsilon, args.max_vars, args.num_extras,
             args.max_component_size, args.verbosity)
