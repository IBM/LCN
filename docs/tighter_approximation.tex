...
cross-family coupling (none, in fact). \texttt{alarm} is the unfavourable case: its cross-family
LMC assertions $(C\perp B,E\mid D,A)$ and $(D\perp B,E\mid C,A)$ span all five atoms and are
\emph{not} RIP-implied by the family tree (the compound node $C\!-\!D$ hides $D$ behind the $\{A\}$
separator). \texttt{CredalJT} drops them (they are inert: a chain-graph product joint satisfies
every LMC by construction), keeps the genuinely-binding small-scope $(B\perp E)$, and so still
runs at treewidth $3$ on \texttt{alarm} while reproducing the brute-force-verified bounds
$P(A){=}[0.087,0.100]$, $P(B){=}[0.100,0.123]$, $P(E){=}[0.050,0.074]$.
