...
\paragraph{The NLP variables and constraints.} One probability vector per cluster:
$q_A\in\Delta(2^3)$ (8 states over $A,B,C$, MSB-first), $q_B\in\Delta(2^2)$ (4 states over
$B,C$), $q_C\in\Delta(2^1)$ (2 states over $C$); each on its own simplex $\sum_i q_\bullet[i]=1$.
On cluster $A$ the engine adds: the Type-1/2 sentence rows (e.g.\ $\mathit{A1}$ as
$0.4\le P(A{=}1)\le0.6$ with $P(A{=}1)=\sum_{b,c}q_A[A{=}1,b,c]$; $\mathit{B1}$ as
$0.1\,P(A{=}1)\le P(B{=}1,A{=}1)\le0.3\,P(A{=}1)$; $\mathit{BC}$ as $P(B{=}1,C{=}1)\le0.05$),
and the bilinear LMC equality for $(B\perp C\mid A)$, namely
$P(B,C,A)\,P(A)=P(B,A)\,P(C,A)$ for each configuration.

\paragraph{Separator-consistency messages.} The two tree edges contribute the Lauritzen
equalities $M_{A\to\{B,C\}}\,q_A = M_{B\to\{B,C\}}\,q_B$ and
$M_{B\to\{C\}}\,q_B = M_{C\to\{C\}}\,q_C$. Written per separator state, the first edge's message
($A\to B$ over $\{B,C\}$) is the four equalities
\begin{align*}
  q_B[B{=}0,C{=}0] &= q_A[A{=}0,B{=}0,C{=}0] + q_A[A{=}1,B{=}0,C{=}0],\\
  q_B[B{=}0,C{=}1] &= q_A[A{=}0,B{=}0,C{=}1] + q_A[A{=}1,B{=}0,C{=}1],\\
  q_B[B{=}1,C{=}0] &= q_A[A{=}0,B{=}1,C{=}0] + q_A[A{=}1,B{=}1,C{=}0],\\
  q_B[B{=}1,C{=}1] &= q_A[A{=}0,B{=}1,C{=}1] + q_A[A{=}1,B{=}1,C{=}1],
\end{align*}
i.e.\ each $q_B$ cell equals the sum of the $q_A$ cells that agree on $(B,C)$---this is the
``message'' $A$ sends to $B$, the marginal of the cluster-$A$ joint onto $\{B,C\}$. The second
edge ($B\to C$ over $\{C\}$) similarly sets
$q_C[C{=}c]=q_B[B{=}0,C{=}c]+q_B[B{=}1,C{=}c]$. Because the tree has the running-intersection
property, these two equalities are enough to guarantee a single global joint exists consistent
with all three cluster marginals.

\paragraph{Reading a marginal: $P(B)$.} For each non-evidence atom the engine swaps in the
objective and solves $\min$/$\max$ over the \emph{same} model. For $B$ the objective is
$P(B{=}1)=\sum_{a,c}q_A[a,B{=}1,c]$, read off the host cluster $A$. The maximization returns
$\overline{P(B{=}1)}=0.72$ at the cluster-$A$ joint
\begin{center}
\begin{tabular}{lcccccccc}
  \toprule
  state $(A,B,C)$ & $000$ & $001$ & $010$ & $011$ & $100$ & $101$ & $110$ & $111$ \\
  $q_A$ & $0$ & $0$ & $0.5751$ & $0.0249$ & $0.2227$ & $0.0573$ & $0.0954$ & $0.0246$ \\
  \bottomrule
\end{tabular}
\end{center}
Here $P(B{=}1)=0.5751+0.0249+0.0954+0.0246=0.72$. The \emph{message} this maximizer sends from
$A$ to $B$, $M_{A\to\{B,C\}}\,q_A$, is
\[
  \big(P(B{=}0,C{=}0),\,P(B{=}0,C{=}1),\,P(B{=}1,C{=}0),\,P(B{=}1,C{=}1)\big)
  = (0.2227,\ 0.0573,\ 0.6705,\ 0.0495),
\]
computed cell-by-cell as the separator equalities above prescribe, e.g.\
$P(B{=}1,C{=}0)=q_A[010]+q_A[110]=0.5751+0.0954=0.6705$. Note the cross-family sentence is
\emph{binding}: $P(B\land C)=q_A[011]+q_A[111]=0.0249+0.0246=0.0495\le0.05$. Without the
joint cluster (i.e.\ over the free product) $B$ could be pushed to $P(B{=}1)$ near $1$ while
$C$ stayed high, but the $0.05$ cap on the shared cell holds it to $0.72$. The minimization
gives $\underline{P(B{=}1)}=0.04$, so $P(B)\in[0.04,0.72]$.
