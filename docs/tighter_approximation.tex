...
\begin{enumerate}
  ...
  \item \textbf{Local credal sets} (\texttt{cn/local\_credal\_sets.py}): for each family
    ...
    $P(\mathrm{ch}_v=c \mid \mathrm{pa}_v=\pi)$. The default \texttt{method="linear"}
    treats each family \emph{in isolation}; \texttt{method="nlp"} additionally imposes
    \emph{pairwise} marginal independence between the parents.
...
\paragraph{D1 --- Constraint back-propagation into local sets (attacks A; drop-in).}
...
$\mathrm{scope}(v)$. Families that thereby acquire bilinear constraints are solved on the
existing hardened ipopt/SCIP \texttt{nlp} path. Closes the part of gap A expressible within
a single family scope; cannot capture constraints spanning atoms in different families.
...
\paragraph{D2 --- Scope merging with a tightness dial (attacks A, partially B).}
Introduce a budget $k$. Merge adjacent families whose combined scope size is $\le k$ into a
...
\paragraph{D3 --- Joint-LP certification of each local bound (attacks A; exact, certified).}
...
the family. This yields $K_v^{\mathrm{true}}$ exactly. Solved with SCIP (the existing
\texttt{solver="scip"} global path) so the bounds carry a global optimality certificate.
